...
class MainViewModel @Inject constructor(
    ...
    private fun getActiveIdsCount(): Int {
        return synchronized(activeIdsLock) { _activeIds.size }
    }
                ...
                when (status) {
                    ...
                    DownloadStatus.CANCELLED -> {
                        ...
                        dao.updateProgress(
                            ...
                            activeThreads = prog.activeThreads
                        )
    ...
    private fun computeNextWindowStart(fromHHMM: String): Long {
        return try {
            ...
            cal.timeInMillis
        } catch (e: Exception) { System.currentTimeMillis() + 60000L }
    }

    private suspend fun checkScheduleWindow() {
        val s = _settings.value
        if (s.defaultScheduleFrom.isBlank() || s.defaultScheduleTo.isBlank()) return

        val windowOpen = scheduleWindowActive()
        val allItems = _state.value.downloads
        val now = System.currentTimeMillis()

        val toPause = allItems.filter { item ->
            (item.status == DownloadStatus.DOWNLOADING ||
                    item.status == DownloadStatus.CONNECTING ||
                    item.status == DownloadStatus.MERGING) &&
                    !windowOpen &&
                    item.scheduleTo.isNotBlank() &&
                    item.progress < 1f
        }

        toPause.forEach { item ->
            Log.d("ScheduleCheck", "Pausing download ${item.id} at ${System.currentTimeMillis()}")
        ...
        val queuedToStart = allItems.filter { item ->
            item.status == DownloadStatus.QUEUED &&
                    item.scheduleFrom.isNotBlank() &&
                    (item.scheduleFrom.toLongOrNull() ?: 0L) <= now
        }

        queuedToStart.forEach { item ->
            val updated = item.copy(
                scheduleFrom = "",
                status = DownloadStatus.CONNECTING,
                startedAt = System.currentTimeMillis()
    ...
    private fun resolveDetectedStreamFilename(
        rawFilename: String,
        mode: DownloadMode,
        outputDir: String
    ): String {
        return if (mode == DownloadMode.HLS) {
            val existing = _state.value.downloads
                .filter { it.status == DownloadStatus.QUEUED || it.status == DownloadStatus.DOWNLOADING ||
                        it.status == DownloadStatus.CONNECTING || it.status == DownloadStatus.MERGING }
                .map { it.filename }
            hlsDownloader.resolveHlsFilenameUnique(rawFilename, outputDir, existing)
        } else {
            rawFilename
        }
    }
    ...
    fun downloadScrapedLinks(links: List<ScrapedLinkUi>) {
        ...
        viewModelScope.launch {
            ...
            links.forEach { link ->
                ...
                val windowNowOpen = if (hasWindow) scheduleWindowActive() else false

                var scheduleFrom = ""
                var scheduleTo = ""

                when {
                    hasWindow && !windowNowOpen -> {
                        scheduleFrom = computeNextWindowStart(s.defaultScheduleFrom).toString()
                ...
                if (schedEpoch != null && schedEpoch > System.currentTimeMillis()) {
                    Scheduler.schedule(app, item.id, schedEpoch)
                }
            }

            hideScraperDialog()
            ForegroundDownloadService.ensureRunning(app)
            ForegroundDownloadService.checkQueue(app)
        }
    }
    ...
    fun queueScrapedLinks(links: List<ScrapedLinkUi>) {
        ...
        viewModelScope.launch {
            ...
            notify("${links.size} link${if (links.size == 1) "" else "s"} added to queue")
        }
    }
    ...
    fun queueClipboardUrls(urls: List<String>) {
        ...
        viewModelScope.launch {
            ...
            notify("${urls.size} link${if (urls.size == 1) "" else "s"} added to queue")
        }
    }
    ...
    fun showVideoDialogLoading(pageUrl: String) {
        if (_isVideoDialogShowing) return
        _isVideoDialogShowing = true
    ...
    fun updateVideoDialogStreams(pageUrl: String, streams: List<VideoStream>) {
        val current = _videoDetectionState.value
        if (current.pageUrl != pageUrl) return 
        _videoDetectionState.value = current.copy(
            streams = streams,
            loading = false,
            detected = true
        )
    ...
    fun saveSettings(s: AppSettings) {
        prefs.edit().apply {
            ...
            putBoolean("autoClipboardDetect", s.autoClipboardDetect)
        }.apply()
        _settings.update { s }
        hideSettingsDialog()
    }
    ...
    private fun loadSettings(): AppSettings = AppSettings(
        ...
        autoClipboardDetect = prefs.getBoolean("autoClipboardDetect", true)
    )
    ...
    fun addToQueue() {
        ...
        viewModelScope.launch {
            ...
            if (schedEpoch != null && schedEpoch > System.currentTimeMillis()) {
                Scheduler.schedule(app, id, schedEpoch)
            } else if (schedEpoch != null && schedEpoch <= System.currentTimeMillis() && scheduleFrom.isNotBlank()) {
                val updated = item.copy(
                    status = DownloadStatus.CONNECTING,
                    scheduleFrom = "",
                    startedAt = System.currentTimeMillis()
    ...
    fun startAllQueued() {
        viewModelScope.launch {
            ...
            val currentActive = getActiveIdsCount()
            val availableSlots = maxC - currentActive
    ...
    private var lastAutoClipboardContent: String = ""

    fun checkClipboardOnResume() {
        if (!_settings.value.autoClipboardDetect) return
        if (_showVideoDialog.value || _state.value.showClipboardDialog) return 

        val urls = clipboardMonitor.readCurrentClipboard(app)
        if (urls.isEmpty()) {
            lastAutoClipboardContent = ""
            return
        }
        val signature = urls.joinToString("|")
        if (signature == lastAutoClipboardContent) return 
        lastAutoClipboardContent = signature
    ...
    fun queueDetectedStream(stream: VideoStream) {
        ...
        val rawFilename = stream.url.substringAfterLast('/').substringBefore('?').ifBlank {
        ...
        viewModelScope.launch {
            val filename = withContext(Dispatchers.IO) {
                resolveDetectedStreamFilename(rawFilename, mode, outputDir)
            }
            val hasWindow = s.defaultScheduleFrom.isNotBlank() && s.defaultScheduleTo.isNotBlank()
            val windowNowOpen = if (hasWindow) scheduleWindowActive() else false
            ...
            when {
                ...
                hasWindow && !windowNowOpen -> {
                    scheduleFrom = nextWindowEpoch?.toString() ?: ""
                    scheduleTo = s.defaultScheduleTo
                }
                else -> {
                    scheduleFrom = ""
            ...
            notify(
                if (schedEpoch != null && schedEpoch > System.currentTimeMillis())
                    "Queued - scheduled to start later"
                else
                    "Added to queue"
            )
    ...
    fun downloadDetectedStream(stream: VideoStream) {
        ...
        viewModelScope.launch {
            ...
            var status = DownloadStatus.CONNECTING
