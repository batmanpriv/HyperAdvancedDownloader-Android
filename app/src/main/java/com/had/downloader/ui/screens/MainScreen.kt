                ...
                SwipeToAction(
                    ...
                    onTap = { downloadItem ->
                        if (state.selectionMode) {
                            vm.toggleSelection(downloadItem.id)
                        }
                    },
                    leftActionLabel = if (item.status == DownloadStatus.DOWNLOADING) "Stop" else "Delete",
                    leftActionIcon = if (item.status == DownloadStatus.DOWNLOADING) Icons.Filled.Stop else Icons.Filled.Delete,
                    rightActionLabel = if (item.status == DownloadStatus.DOWNLOADING) "Pause" else "Resume",
                    rightActionIcon = if (item.status == DownloadStatus.DOWNLOADING) Icons.Filled.Pause else Icons.Filled.PlayArrow
                ) { downloadItem ->
                    DownloadCard(
                        item = downloadItem,
                        ...
                        onStop = { vm.stopDownload(downloadItem.id) },
                        onDelete = { vm.deleteDownload(downloadItem) },
                        onDeleteWithFile = { vm.deleteDownloadWithFile(downloadItem) },
                        onRetry = { vm.retryDownload(downloadItem) },
                        onViewMode = { mode -> vm.setThreadViewMode(downloadItem.id, mode) },
                        onLongPress = { vm.toggleSelection(downloadItem.id) },
                        onTap = {
                            if (state.selectionMode) {
                                vm.toggleSelection(downloadItem.id)
                            }
...
fun ScraperDialog(
    ...
    val filteredLinks = remember(visibleLinks, nameFilter, typeFilter, selectedQuality, selectedExt) {
                    ...
                    filteredLinks.forEach { link ->
                        ...
                        Row(
                            ...
                            Column(Modifier.weight(1f)) {
                                ...
                                if (link.quality != VideoQualityType.UNKNOWN) {
                                    Row(
