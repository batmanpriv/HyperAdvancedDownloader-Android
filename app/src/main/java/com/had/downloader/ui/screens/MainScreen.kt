...
enum class DrawerDestination(
    ...
    private val labelFa: String,
    val icon: androidx.compose.ui.graphics.vector.ImageVector
) {
    ALL("All Downloads", "همه‌ی دانلودها", Icons.Outlined.Inbox),
    ACTIVE("Active", "فعال", Icons.Outlined.CloudDownload),
    DONE("Completed", "تکمیل‌شده", Icons.Outlined.CheckCircle),
    FAILED("Failed", "ناموفق", Icons.Outlined.ErrorOutline),
    BROWSER("Browser", "مرورگر", Icons.Outlined.Language),
    TORRENT("Torrent", "تورنت", Icons.Outlined.CloudDownload),
    REMOTE_SERVER("Remote Server", "سرور ریموت", Icons.Outlined.Wifi),
    ANALYTICS("Analytics", "آمار", Icons.Outlined.BarChart),
    SETTINGS("Settings", "تنظیمات", Icons.Outlined.Settings),
    ABOUT("About", "درباره", Icons.Outlined.Info),
    WEB_ARCHIVE("Web Archive", "آرشیو وب", Icons.Outlined.Archive),
    GUIDE("Guide", "راهنما", Icons.Outlined.MenuBook);

    val label: String get() = t(labelEn, labelFa)
}
...
fun MainScreen(vm: MainViewModel = hiltViewModel()) {
    ...
    val hlsDownloader = remember { HlsDownloader() }
    ...
    val activeDownload = state.downloads.firstOrNull {
    ...
    ModalNavigationDrawer(
        ...
        gesturesEnabled = currentDest != DrawerDestination.BROWSER,
        drawerContent = {
            HADDrawerContent(
                current = currentDest,
                stats = state.stats,
                onSelect = { dest ->
                    ...
                    when (dest) {
                        ...
                        DrawerDestination.BROWSER -> {
                            vm.navigateToTab(6)
                        }
                        DrawerDestination.TORRENT -> {
                            vm.navigateToTab(7)
                        }
                        DrawerDestination.REMOTE_SERVER -> {
                            vm.navigateToTab(8)
                        }
                        DrawerDestination.ANALYTICS -> {
                            vm.navigateToTab(9)
                        }
                        DrawerDestination.WEB_ARCHIVE -> {
                            vm.navigateToTab(10)
                        }
        ...
        Box(modifier = Modifier.fillMaxSize().background(SpaceBlack)) {
                    ...
                    if (showFab) {
                        Column(
                            ...
                            SmallFloatingActionButton(
                                ...
                                containerColor = ElevatedSurf,
                                contentColor = CyanPrimary,
                                shape = CircleShape
                            ) {
                                Icon(
                                    imageVector = Icons.Outlined.QrCodeScanner,
                                    contentDescription = t("Scan QR code", "اسکن کد QR"),
                            ...
                            SmallFloatingActionButton(
                                ...
                                Icon(
                                    ...
                                    contentDescription = t("Scrape links", "استخراج لینک‌ها"),
                                    modifier = Modifier.size(20.dp)
                                )
                            }
                            FloatingActionButton(
                                ...
                                Icon(
                                    ...
                                    contentDescription = t("New download", "دانلود جدید"),
                ...
                Column(
                    ...
                    when (currentDest) {
                        ...
                        DrawerDestination.FAILED -> {
                            ...
                            DownloadListContent(state, vm, onShowQR = { showQRCodeDialog = true })
                        }
                        DrawerDestination.BROWSER -> BrowserTabScreen(vm, hlsDownloader)
            ...
            if (state.showAddDialog) {
                AddDownloadDialog(
                    ...
                    onShowQR = { showQRCodeDialog = true }
                )
            }
            ...
            if (state.showScraperDialog) {
                ScraperDialog(
                    ...
                    onScrape = { url, quality -> vm.scrape(url, quality) },
                    ...
                    onClearCache = vm::clearScraperCache,
            ...
            if (vm.showVideoDialog.collectAsState().value) {
                val videoState = vm.videoDetectionState.collectAsState().value
                VideoQualitySelectorDialog(
                    streams = videoState.streams,
                    pageUrl = videoState.pageUrl,
                    loading = videoState.loading,
                    onDownload = { stream ->
                        vm.downloadDetectedStream(stream)
                    },
                    onQueue = { stream ->
                        vm.queueDetectedStream(stream)
                    },
                    onDismiss = vm::closeVideoDialog
                )
            }

            if (showQRCodeDialog && form.url.isNotBlank()) {
                QRCodeDialog(
                    url = form.url,
                    filename = form.fileSizeResult?.filename ?: "",
            ...
            if (currentDest == DrawerDestination.BROWSER && activeDownload != null) {
                MiniPlayerView(
                    download = activeDownload,
                    onExpand = {
                        currentDest = DrawerDestination.ALL
                        vm.setTab(0)
                    },
                    onMinimize = { },
                    onCancel = { vm.stopDownload(activeDownload.id) },
                    modifier = Modifier
...
private fun StartAllQueuedBar(count: Int, onStartAll: () -> Unit) {
    Row(
        ...
        Button(
            ...
            Text(t("Start All", "شروع همه"), fontSize = 12.sp, fontWeight = FontWeight.Bold)
        }
    }
}
...
private fun HADDrawerContent(
    ...
    ModalDrawerSheet(
        ...
        Column(
                ...
                Column {
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        ...
                        Column {
                            ...
                            Text(
                                t("Hyper Advanced Downloader", "دانلودر فوق‌پیشرفته"), color = CyanPrimary,
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                        DrawerStatChip("${stats.active}", t("active", "فعال"), CyanPrimary)
                        DrawerStatChip("${stats.completed}", t("done", "تکمیل"), GreenSuccess)
                        DrawerStatChip(stats.totalDownloaded.toHumanSize(), t("saved", "ذخیره‌شده"), PurpleAccent)
                    }
                }
            }
            ...
            DrawerSectionLabel(t("DOWNLOADS", "دانلودها"))
            listOf(
                DrawerDestination.ALL,
                DrawerDestination.ACTIVE,
                DrawerDestination.DONE,
            ...
            DrawerSectionLabel(t("TOOLS", "ابزارها"))
            DrawerItem(
            ...
            DrawerSectionLabel(t("APP", "برنامه"))
            listOf(
                DrawerDestination.ABOUT,
                DrawerDestination.SETTINGS,
                DrawerDestination.GUIDE
            ...
            Box(modifier = Modifier.fillMaxWidth().padding(16.dp)) {
                Text(
                    t("v2.0.0  •  Android 8+", "نسخه ۲.۰.۰  •  اندروید ۸+"),
            ...
            if (showDownloadActions) {
                ...
                IconButton(onClick = onPasteClick) {
                    Icon(
                        ...
                        contentDescription = t("Paste URL", "چسباندن لینک"),
                        tint = CyanPrimary,
    ...
    Box(
        ...
        Column(horizontalAlignment = Alignment.CenterHorizontally) {
            ...
            if (tab == 0) {
                ...
                Text(
                    t("Tap + to add a download", "برای افزودن دانلود، روی + بزن"),
    ...
    AlertDialog(
        ...
        title = {
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text(t("New Download", "دانلود جدید"), fontWeight = FontWeight.Bold, color = TextPrimary)
                Spacer(Modifier.weight(1f))
                Box(
                    modifier = Modifier
            ...
            Column(
                ...
                if (scheduleWindowActive) {
                    Row(
                        ...
                        Text(
                            t("Schedule window: ", "بازه‌ی زمان‌بندی: ") + "${settings.defaultScheduleFrom} → ${settings.defaultScheduleTo}",
                        ...
                        Row {
                            IconButton(
                                ...
                                Icon(
                                    ...
                                    null,
                                    tint = if (form.url.isNotBlank()) CyanPrimary else TextMuted,
                                    modifier = Modifier.size(20.dp)
                                )
                            }
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    HADTextField(
                        ...
                        label = t("Threads", "رشته"), placeholder = "4",
                ...
                AnimatedVisibility(visible = showAdvanced) {
                    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        HADTextField(
                            ...
                            label = t("Proxy", "پراکسی"), placeholder = "socks5://127.0.0.1:1080",
                            icon = Icons.Outlined.VpnLock
                        )
                        HADTextField(
                            value = form.userAgent,
                            onValueChange = { onUpdate(form.copy(userAgent = it)) },
                            label = t("User-Agent", "یوزر ایجنت"), placeholder = "Mozilla/5.0 ...",
                            icon = Icons.Outlined.Computer
                        )
                        HADTextField(
                            value = form.cookies,
                            onValueChange = { onUpdate(form.copy(cookies = it)) },
                            label = t("Cookies", "کوکی‌ها"), placeholder = "session=abc; token=xyz",
                            icon = Icons.Outlined.Cookie
                        )
                        HADTextField(
                            value = form.customHeaders,
                            onValueChange = { onUpdate(form.copy(customHeaders = it)) },
                            label = t("Headers (key: value per line)", "هدرها (هر خط یک key: value)"),
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            HADTextField(
                                ...
                                label = t("Method", "متد"), placeholder = "GET",
                            ...
                            HADTextField(
                                ...
                                label = t("Max KB/s (0=∞)", "حداکثر KB/s (۰=نامحدود)"), placeholder = "0",
                                icon = Icons.Outlined.Speed,
                                keyboardType = KeyboardType.Number,
                        ...
                        HADTextField(
                            ...
                            label = t("Mirror URLs (one per line)", "لینک‌های میرور (هر خط یکی)"),
                        ...
                        Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                            HADTextField(
                                ...
                                label = t("Hash algo", "الگوریتم هش"), placeholder = "SHA-256",
                            ...
                            HADTextField(
                                ...
                                label = t("Expected hash", "هش مورد انتظار"), placeholder = "a1b2c3...",
                                icon = Icons.Outlined.Tag,
                                modifier = Modifier.weight(0.65f)
                            )
                        }
                        ToggleChip(t("Resume on retry", "ادامه در تلاش مجدد"), form.useResume) {
        ...
        confirmButton = {
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                OutlinedButton(
                    ...
                    Spacer(Modifier.width(4.dp))
                    Text(t("Queue", "صف"), fontSize = 12.sp, fontWeight = FontWeight.Medium)
                }
                Button(
                    onClick = onStart,
                    enabled = form.url.isNotBlank(),
                    ...
                    Spacer(Modifier.width(4.dp))
                    Text(t("Start", "شروع"), fontWeight = FontWeight.Bold)
                }
            }
        },
        dismissButton = {
            TextButton(onClick = onDismiss) { Text(t("Cancel", "انصراف"), color = TextSecondary) }
        }
    )
}
...
private fun FileSizeInfoCard(result: FileSizeResult?, loading: Boolean) {
    AnimatedVisibility(
        ...
        val sizeText = if (result.sizeBytes > 0) result.sizeBytes.toHumanSize() else t("Unknown size", "حجم نامشخص")
...
fun ScraperDialog(
    ...
    maxConcurrent: Int,
    onScrape: (String, VideoQualityType?) -> Unit,
    ...
    var pageUrl by remember { mutableStateOf(result.pageUrl) }
    var nameFilter by remember { mutableStateOf("") }
    var typeFilter by remember { mutableStateOf("") }
    var selectedQuality by remember { mutableStateOf<VideoQualityType?>(null) }
    var selectedExt by remember { mutableStateOf<String?>(null) }
    var selectedSubDub by remember { mutableStateOf<String?>(null) }
    ...
    val visibleLinks = remember(result.links) {
        result.links.filter { link ->
            link.linkType != LinkType.OTHER &&
                    link.url.startsWith("http")
        }
    }

    val availableQualities = remember(result.qualityOptions) {
        result.qualityOptions.filter { it != VideoQualityType.UNKNOWN }
            .distinct()
            .sortedByDescending { it.maxHeight }
    }

    val availableExtensions = remember(visibleLinks) {
        visibleLinks.mapNotNull { link ->
            val ext = link.url.substringAfterLast('.').substringBefore('?').lowercase()
            if (ext.isNotBlank() && !ext.contains('/') && ext.length <= 6) ext else null
        }
            .distinct()
            .sorted()
    }

    val availableSubDub = remember(visibleLinks) {
        visibleLinks.mapNotNull { link ->
            link.parsedInfo.subDub.takeIf { it.isNotBlank() }
        }.distinct().sorted()
    }

    val availableSource = remember(visibleLinks) {
        visibleLinks.mapNotNull { link ->
            link.parsedInfo.source.takeIf { it.isNotBlank() }
        }.distinct().sorted()
    }

    val filteredLinks = remember(visibleLinks, nameFilter, typeFilter, selectedQuality, selectedExt, selectedSubDub, selectedSource) {
        visibleLinks.filter { link ->
            ...
            val qualityOk = selectedQuality == null || link.quality == selectedQuality
            val ext = link.url.substringAfterLast('.').substringBefore('?').lowercase()
            val extOk = selectedExt == null || ext == selectedExt
            val subDubOk = selectedSubDub == null || link.parsedInfo.subDub.equals(selectedSubDub, ignoreCase = true)
            val sourceOk = selectedSource == null || link.parsedInfo.source.equals(selectedSource, ignoreCase = true)

            nameOk && typeOk && qualityOk && extOk && subDubOk && sourceOk
        }
    }
    ...
    AlertDialog(
        ...
        title = {
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Spacer(Modifier.width(8.dp))
                Text(t("Link Scraper", "استخراج‌کننده‌ی لینک"), fontWeight = FontWeight.Bold, color = TextPrimary)
                ...
                if (result.isCached) {
                    ...
                    Box(
                        modifier = Modifier
                            .background(OrangeWarn.copy(alpha = 0.12f), RoundedCornerShape(20.dp))
                            .padding(horizontal = 6.dp, vertical = 2.dp)
                    ) {
                        Text(
                            t("Cached", "کش‌شده"),
                            color = OrangeWarn, fontSize = 8.sp,
                            fontFamily = FontFamily.Monospace, fontWeight = FontWeight.Bold
                        )
                    }
                }
        ...
        text = {
            Column(
                ...
                Row(
                    ...
                    if (result.isCached) {
                        IconButton(
                            ...
                            modifier = Modifier
                                .size(40.dp)
                                .background(RedError.copy(alpha = 0.1f), RoundedCornerShape(10.dp))
                        ) {
                            Icon(
                                Icons.Filled.Delete,
                                contentDescription = t("Clear Cache", "پاک کردن کش"),
                                tint = RedError,
                                modifier = Modifier.size(20.dp)
                            )
                        }
                ...
                Row(
                    ...
                    Button(
                        onClick = { onScrape(pageUrl, selectedQuality) },
                        ...
                        if (result.loading) {
                            ...
                            Spacer(Modifier.width(8.dp))
                            Text(t("Scraping...", "در حال استخراج..."), fontWeight = FontWeight.Bold)
                        } else {
                            Icon(Icons.Outlined.TravelExplore, null, modifier = Modifier.size(16.dp))
                            Spacer(Modifier.width(8.dp))
                            Text(if (result.isCached) t("Re-scrape", "استخراج دوباره") else t("Scrape Links", "استخراج لینک‌ها"), fontWeight = FontWeight.Bold)
                        }
                    }

                    if (result.isCached && visibleLinks.isNotEmpty()) {
                        OutlinedButton(
                            onClick = onClearCache,
                ...
                if (visibleLinks.isNotEmpty()) {
                    Row(
                        ...
                        Text(
                            t("Max $maxConcurrent simultaneous · rest queued automatically", "حداکثر $maxConcurrent هم‌زمان · بقیه به‌صورت خودکار در صف"),
                            ...
                            Row(
                                ...
                                availableQualities.forEach { quality ->
                                    QualityChip(
                                        ...
                                        onClick = {
                                            selectedQuality = if (selectedQuality == quality) null else quality
                                        }
                                    )
                                }
                            }
                    ...
                    if (availableExtensions.isNotEmpty()) {
                        Column(
                            ...
                            Row(
                                ...
                                ExtChip(
                                    label = t("All", "همه"),
                                    selected = selectedExt == null,
                                    onClick = { selectedExt = null }
                                )
                                availableExtensions.forEach { ext ->
                                    ExtChip(
                                        ...
                                        onClick = {
                                            selectedExt = if (selectedExt == ext) null else ext
                                        }
                                    )
                                }
                            }
                    ...
                    if (availableSubDub.isNotEmpty()) {
                        Column(
                            modifier = Modifier.fillMaxWidth(),
                            verticalArrangement = Arrangement.spacedBy(4.dp)
                        ) {
                            Text(
                                t("Sub/Dub:", "زیرنویس/دوبله:"),
                                color = TextSecondary,
                                fontSize = 10.sp,
                                fontWeight = FontWeight.Medium,
                                fontFamily = FontFamily.Monospace,
                            ...
                            Row(
                                ...
                                SubDubChip(
                                    label = t("All", "همه"),
                                    selected = selectedSubDub == null,
                                    onClick = { selectedSubDub = null }
                                )
                                availableSubDub.forEach { value ->
                                    SubDubChip(
                                        label = value,
                                        selected = selectedSubDub == value,
                                        onClick = {
                                            selectedSubDub = if (selectedSubDub == value) null else value
                                        }
                                    )
                                }
                            }
                    ...
                    if (availableSource.isNotEmpty()) {
                        Column(
                            ...
                            Row(
                                ...
                                availableSource.forEach { value ->
                                    SourceChip(
                                        label = value,
                                        selected = selectedSource == value,
                                        onClick = {
                                            selectedSource = if (selectedSource == value) null else value
                                        }
                                    )
                                }
                            }
                    ...
                    if (typeGroups.size > 1) {
                        ...
                        Row(
                            ...
                            TypeFilterChip(label = t("ALL", "همه"), selected = typeFilter.isBlank()) {
                                typeFilter = ""
                            }
                    ...
                    filteredLinks.forEach { link ->
                        ...
                        val typeColor = getTypeLabelColor(link.typeLabel)
                        val subDubColor = when {
                            link.parsedInfo.subDub.contains("Dub", ignoreCase = true) -> OrangeWarn
                            link.parsedInfo.subDub.contains("Sub", ignoreCase = true) -> GreenSuccess
                            else -> TextMuted
                        }
                        ...
                        Row(
                            ...
                            Spacer(Modifier.width(8.dp))
                            Column(Modifier.weight(1f)) {
                                if (link.parsedInfo.title.isNotBlank()) {
                                    Text(
                                        link.parsedInfo.title,
                                ...
                                Row(
                                    ...
                                    if (link.parsedInfo.season != null && link.parsedInfo.episode != null) {
                                        Text(
                                            "S${link.parsedInfo.season.toString().padStart(2, '0')}E${link.parsedInfo.episode.toString().padStart(2, '0')}",
                                            color = CyanPrimary,
                                            fontSize = 10.sp,
                                            fontFamily = FontFamily.Monospace,
                                            fontWeight = FontWeight.Bold
                                    ...
                                    if (link.parsedInfo.quality.isNotBlank()) {
                                        Text(
                                            "• ${link.parsedInfo.quality}",
                                            color = getQualityColor(link.quality),
                                            fontSize = 9.sp,
                                            fontFamily = FontFamily.Monospace
                                        )
                                    }
                                    if (link.parsedInfo.source.isNotBlank()) {
                                        Text(
                                            "• ${link.parsedInfo.source}",
                                            color = PurpleAccent,
                                            fontSize = 9.sp,
                                            fontFamily = FontFamily.Monospace
                                        )
                                    }
                                    if (link.parsedInfo.subDub.isNotBlank()) {
                                        Text(
                                            "• ${link.parsedInfo.subDub}",
                                            color = subDubColor,
                                            fontSize = 9.sp,
                                            fontFamily = FontFamily.Monospace
                                        )
                                    }
                                    if (link.parsedInfo.group.isNotBlank()) {
                                        Text(
                                            "• ${link.parsedInfo.group}",
                                            color = TextMuted,
                                            fontSize = 8.sp,
                                            fontFamily = FontFamily.Monospace
                                        )
                                ...
                                if (link.quality != VideoQualityType.UNKNOWN && link.parsedInfo.title.isBlank()) {
                                    Row(
                                        ...
                                        Box(
                                            modifier = Modifier
                                                .size(6.dp)
                                                .background(getQualityColor(link.quality), CircleShape)
                                        )
                                        Text(
                                            link.quality.label,
                                            color = TextMuted,
                            ...
                            Column(
                                ...
                                if (ext.isNotBlank() && !ext.contains('/') && !ext.contains('?')) {
                                    Box(
                                        modifier = Modifier
                                            .background(getTypeLabelColor(link.typeLabel).copy(alpha = 0.12f), RoundedCornerShape(4.dp))
                                            .padding(horizontal = 5.dp, vertical = 2.dp)
                                    ) {
                                        Text(
                                            ".${ext}", color = getTypeLabelColor(link.typeLabel), fontSize = 9.sp,
                                            fontFamily = FontFamily.Monospace, fontWeight = FontWeight.Bold
                                        )
                                    }
                                }
                                Box(
                                    modifier = Modifier
                                        .background(getTypeLabelColor(link.typeLabel).copy(alpha = 0.08f), RoundedCornerShape(4.dp))
                                        .padding(horizontal = 5.dp, vertical = 2.dp)
                                ) {
                                    Text(
                                        link.typeLabel,
                                        color = getTypeLabelColor(link.typeLabel).copy(alpha = 0.8f),
                                        fontSize = 8.sp,
                                        fontFamily = FontFamily.Monospace
                                    )
                                }
        ...
        dismissButton = {
            TextButton(onClick = onDismiss) { Text(t("Close", "بستن"), color = TextSecondary) }
        }
    )
}

@Composable
private fun ExtChip(label: String, selected: Boolean, onClick: () -> Unit) {
    val color = if (selected) CyanPrimary else TextMuted
    Box(
        modifier = Modifier
            .clickable { onClick() }
            .background(
...
@Composable
private fun SubDubChip(label: String, selected: Boolean, onClick: () -> Unit) {
    val color = when {
        label.contains("Dub", ignoreCase = true) -> OrangeWarn
        label.contains("Sub", ignoreCase = true) -> GreenSuccess
        else -> PurpleAccent
    }
    Box(
        modifier = Modifier
            .clickable { onClick() }
            .background(
                if (selected) color.copy(alpha = 0.2f) else ElevatedSurf,
                RoundedCornerShape(12.dp)
            )
            .border(
                1.dp,
...
private fun SourceChip(label: String, selected: Boolean, onClick: () -> Unit) {
    val color = if (selected) PurpleAccent else TextMuted
    Box(
        modifier = Modifier
            .clickable { onClick() }
            .background(
                if (selected) PurpleAccent.copy(alpha = 0.2f) else ElevatedSurf,
                RoundedCornerShape(12.dp)
            )
            .border(
                1.dp,
                if (selected) PurpleAccent else BorderColor,
                RoundedCornerShape(12.dp)
            )
            .padding(horizontal = 10.dp, vertical = 5.dp)
    ) {
...
@Composable
private fun getQualityColor(quality: VideoQualityType): Color {
    return when (quality) {
        VideoQualityType.UHD_4K -> CyanPrimary
        VideoQualityType.QHD_2K -> CyanPrimary.copy(alpha = 0.7f)
        VideoQualityType.FHD_1080 -> GreenSuccess
        VideoQualityType.HD_720 -> OrangeWarn
        VideoQualityType.SD_480 -> OrangeWarn.copy(alpha = 0.7f)
        VideoQualityType.SD_360 -> TextSecondary
        VideoQualityType.LOW_240 -> TextSecondary.copy(alpha = 0.5f)
        VideoQualityType.AUDIO_ONLY -> PurpleAccent
        else -> TextMuted
    }
}
...
private fun QualityChip(label: String, selected: Boolean, onClick: () -> Unit) {
    val color = when {
        label.contains("4K") || label.contains("2160") -> CyanPrimary
        label.contains("1080") -> GreenSuccess
        label.contains("720") -> OrangeWarn
        label.contains("Audio") -> PurpleAccent
        else -> TextSecondary
    }
...
@Composable
private fun getTypeLabelColor(typeLabel: String): Color = when (typeLabel) {
    "VIDEO" -> GreenSuccess
    "AUDIO" -> PurpleAccent
