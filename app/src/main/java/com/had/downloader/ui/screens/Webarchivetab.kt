...
private fun ArchiveEmptyState(onNew: () -> Unit) {
    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
        Column(
            ...
            Text(
                t("Backup any website for offline use", "از هر وب‌سایتی نسخه‌ی آفلاین بگیر"),
            ...
            Button(
                ...
                Text(t("New Archive", "آرشیو جدید"), fontWeight = FontWeight.Bold)
            }
        }
    }
}
...
fun ArchiveSessionCard(
    ...
    Surface(
        ...
        Column(Modifier.padding(14.dp)) {
            ...
            if (isRunning) {
                val totalPages = if (progress.totalPages > 0) progress.totalPages else 1L
                val pagesDone = progress.pages.coerceAtMost(totalPages)
                val pageProgress = if (totalPages > 0) pagesDone.toFloat() / totalPages else 0f
                ...
                Row(
                    ...
                    Text(
                        "${pagesDone}/${if (progress.totalPages > 0) progress.totalPages else "?"} pages",
                        color = TextMuted,
                        fontSize = 9.sp,
                        fontFamily = FontFamily.Monospace
                    )
                ...
                LinearProgressIndicator(
                    progress = { pageProgress.coerceIn(0f, 1f) },
            ...
            Row(
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                    ArchiveInfoChip("${progress.pages}", t("pages", "صفحه"), GreenSuccess)
                    ArchiveInfoChip("${progress.assets}", t("assets", "فایل"), CyanPrimary)
                    ArchiveInfoChip(progress.bytes.toHumanSize(), t("saved", "ذخیره‌شده"), PurpleAccent)
                }
                if (progress.errors > 0) {
            ...
            AnimatedVisibility(visible = expanded) {
                Column {
                    ...
                    Row(
                        ...
                        ArchiveModeChip(
                            if (session.config.mode == ArchiveCrawlMode.SINGLE_PAGE) t("Single Page", "تک‌صفحه") else t("Full Site", "کل سایت"),
                            PurpleAccent
                        )
                        if (session.config.mode == ArchiveCrawlMode.FULL_SITE)
                            ArchiveModeChip(t("Max ", "حداکثر ") + "${session.config.maxPages} " + t("pages", "صفحه"), TextSecondary)
                        ArchiveModeChip("${session.config.concurrency} workers", CyanPrimary)
                        if (session.config.minifyOutput) ArchiveModeChip(t("Minify", "فشرده"), OrangeWarn)
                        if (session.config.downloadExternal) ArchiveModeChip(t("External", "خارجی"), OrangeWarn)
                    }
...
private fun ArchiveStatusBadge(status: String) {
    val (label, color) = when {
        status == "COMPLETED" -> t("DONE", "تمام") to GreenSuccess
        status.startsWith("FAILED") -> t("FAILED", "ناموفق") to RedError
        status == "RUNNING" -> t("RUNNING", "درحال اجرا") to PurpleAccent
        else -> t("IDLE", "بی‌کار") to TextMuted
    }
    ...
    AlertDialog(
        ...
        title = {
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Spacer(Modifier.width(8.dp))
                Text(t("New Web Archive", "آرشیو وب جدید"), fontWeight = FontWeight.Bold, color = TextPrimary)
            }
        },
        text = {
            Column(
                ...
                Row(
                    ...
                    listOf(
                        ArchiveCrawlMode.SINGLE_PAGE to t("Single Page", "تک‌صفحه"),
                        ArchiveCrawlMode.FULL_SITE to t("Full Site", "کل سایت")
                    ).forEach { (m, label) ->
                        val selected = mode == m
                        Box(
                            modifier = Modifier
                ...
                AnimatedVisibility(visible = showAdvanced) {
                    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
                        ...
                        Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
                            ArchiveToggle(t("Download external assets", "دانلود فایل‌های خارجی"), downloadExternal) { downloadExternal = it }
                            ArchiveToggle(t("Minify HTML output", "فشرده‌سازی خروجی HTML"), minify) { minify = it }
                            ArchiveToggle(t("Crawl iframes", "خزیدن در iframeها"), crawlIframes) { crawlIframes = it }
                            ArchiveToggle(t("Handle hash routes (SPA)", "پشتیبانی از مسیرهای hash (SPA)"), crawlHashRoutes) { crawlHashRoutes = it }
                            ArchiveToggle(t("Follow meta-refresh", "دنبال کردن meta-refresh"), followMeta) { followMeta = it }
                        }
                    }
                }
            }
        ...
        confirmButton = {
            Button(
                ...
                Spacer(Modifier.width(4.dp))
                Text(t("Start Archive", "شروع آرشیو"), fontWeight = FontWeight.Bold)
            }
        },
        dismissButton = {
