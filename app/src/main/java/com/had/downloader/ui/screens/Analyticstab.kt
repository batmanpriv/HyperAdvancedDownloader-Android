...
import com.had.downloader.ui.utils.ConnectionType
import com.had.downloader.ui.utils.NetworkInfoProvider
...
private fun NetworkInfoSection() {
    ...
    var info by remember { mutableStateOf(NetworkInfoProvider.current(context)) }

    LaunchedEffect(Unit) {
        while (true) {
            info = NetworkInfoProvider.current(context)
            delay(3000)
        }
    }

    val (icon, label) = when (info.type) {
        ConnectionType.WIFI -> Icons.Outlined.Wifi to "Wi-Fi"
        ConnectionType.MOBILE -> Icons.Outlined.SignalCellularAlt to (info.carrierName ?: "Mobile data")
        ConnectionType.ETHERNET -> Icons.Outlined.Cable to "Ethernet"
        ConnectionType.VPN -> Icons.Outlined.VpnLock to "VPN"
        ConnectionType.OTHER -> Icons.Outlined.SignalWifiStatusbarConnectedNoInternet4 to "Connected"
        ConnectionType.NONE -> Icons.Outlined.SignalWifiOff to "No connection"
    }
    val tint = if (info.type == ConnectionType.NONE) RedError else CyanPrimary
    ...
    Surface(
        ...
        Row(
            ...
            Column {
                ...
                Text(
                    if (info.type == ConnectionType.MOBILE) t("Mobile data", "دیتای موبایل") else t("Connection type", "نوع اتصال"),
                    color = TextMuted,
                    fontSize = 10.sp
                )
            }
...
private fun OverallStatsSection(stats: OverallStats) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
        ...
        Row(
            ...
            StatCard2(
                label = t("Total Saved", "حجم ذخیره‌شده"),
                value = stats.totalBytes.toHumanSize(),
                color = CyanPrimary,
                modifier = Modifier.weight(1f)
            )
            StatCard2(
                label = t("Avg Speed", "سرعت میانگین"),
                value = stats.avgSpeed.toSpeedString(),
                color = PurpleAccent,
                modifier = Modifier.weight(1f)
            )
            StatCard2(
                label = t("Peak Speed", "بیشترین سرعت"),
            ...
            events.take(20).forEach { event ->
                ...
                Row(
                    ...
                    Box(
                        ...
                        Text(
                            if (event.success) t("OK", "موفق") else t("FAIL", "ناموفق"),
