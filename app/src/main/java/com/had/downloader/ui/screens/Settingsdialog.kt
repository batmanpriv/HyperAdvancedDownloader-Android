    ...
    AlertDialog(
        ...
        title = {
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text(t("Settings", "تنظیمات"), fontWeight = FontWeight.Black, color = TextPrimary, fontSize = 18.sp)
            }
        },
        text = {
            Column(
                ...
                SettingsSection(
                    title    = t("Download Defaults", "پیش‌فرض‌های دانلود"),
                    icon     = Icons.Outlined.CloudDownload,
                    color    = CyanPrimary,
                    summary  = t("${threads}T  •  ${maxParallel} parallel", "${threads} رشته  •  ${maxParallel} هم‌زمان"),
                    ...
                    SettingsTextField(
                        ...
                        label         = t("Default Save Folder", "پوشه‌ی ذخیره‌ی پیش‌فرض"),
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        SettingsTextField(
                            ...
                            label         = t("Threads / file", "رشته / هر فایل"),
                            placeholder   = "4",
                        ...
                        SettingsTextField(
                            ...
                            label         = t("Max parallel", "حداکثر هم‌زمان"),
                            placeholder   = "2",
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        SettingsTextField(
                            ...
                            label         = t("Timeout (s)", "زمان انتظار (ثانیه)"),
                            placeholder   = "30",
                ...
                SettingsSection(
                    title    = t("Network", "شبکه"),
                    icon     = Icons.Outlined.VpnLock,
                    color    = PurpleAccent,
                    summary  = if (proxy.isBlank()) t("No proxy", "بدون پراکسی") else proxy.take(22),
                    ...
                    SettingsTextField(
                        ...
                        label         = t("Default Proxy", "پراکسی پیش‌فرض"),
                ...
                SettingsSection(
                    title    = t("Schedule Window", "بازه‌ی زمان‌بندی"),
                    icon     = Icons.Outlined.Schedule,
                    color    = OrangeWarn,
                    summary  = if (scheduleEnabled)
                        "%02d:%02d → %02d:%02d".format(schedStartHour, schedStartMin, schedEndHour, schedEndMin)
                    else t("Disabled — all downloads run anytime", "غیرفعال — دانلودها هر زمان اجرا می‌شوند"),
                ...
                SettingsSection(
                    ...
                    color    = GreenSuccess,
                    summary  = buildString {
                        if (saveSession)   append(t("Resume  ", "ادامه  "))
                        if (enableGzip)    append(t("Gzip  ", "Gzip  "))
                        if (notifications) append(t("Notif", "اعلان"))
                    }.trim().ifBlank { t("All off", "همه خاموش") },
                    expanded = openSection == "options",
                    onToggle = { openSection = if (openSection == "options") null else "options" }
                ) {
                    SettingsToggle(t("Save Session (Resume)", "ذخیره‌ی نشست (ادامه‌ی دانلود)"), saveSession)   { saveSession = it }
                    SettingsToggle(t("Enable Gzip", "فعال‌سازی Gzip"),           enableGzip)    { enableGzip = it }
                    SettingsToggle(t("Show Notifications", "نمایش اعلان‌ها"),    notifications) { notifications = it }
                }

                SettingsSection(
                    title    = t("Appearance", "ظاهر برنامه"),
                    icon     = Icons.Outlined.Palette,
                    color    = ThemeManager.current.palette.purpleAccent,
                    summary  = t(
                        "${LanguageManager.current.displayNameNative} • ${ThemeManager.current.nameEn}",
                        "${LanguageManager.current.displayNameNative} • ${ThemeManager.current.nameFa}"
                    ),
                    expanded = openSection == "appearance",
                    onToggle = { openSection = if (openSection == "appearance") null else "appearance" }
                ) {
        ...
        confirmButton = {
            Button(
                ...
                Text(t("Save", "ذخیره"), fontWeight = FontWeight.Bold)
            }
        },
        dismissButton = {
...
private fun ScheduleWindowSection(
    ...
    Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
        ...
        Row(
            ...
            Column {
                Text(t("Time Window", "بازه‌ی زمانی"), color = TextPrimary, fontSize = 14.sp, fontWeight = FontWeight.SemiBold)
                Text(
                    if (enabled) t("Downloads only run within this time range", "دانلودها فقط در این بازه‌ی زمانی اجرا می‌شوند")
                    else t("Downloads run at any time", "دانلودها هر زمان اجرا می‌شوند"),
            ...
            Column(verticalArrangement = Arrangement.spacedBy(16.dp)) {
                ...
                if (isOvernight) {
                    Row(
                        ...
                        Text("🌙", fontSize = 13.sp)
                        Text(
                            t("Overnight window · crosses midnight · $durationLabel total",
                              "بازه‌ی شبانه · از نیمه‌شب می‌گذرد · مجموعاً $durationLabel"),
                            color    = OrangeWarn,
                            fontSize = 11.sp
                        )
                    }
        ...
        if (!enabled) {
            Row(
                ...
                Text(
                    t("Enable to restrict downloads to specific hours only", "برای محدود کردن دانلودها به ساعات خاص، فعال کن"),
...
private fun SpeedField(value: String, onChange: (String) -> Unit) {
    val speedLabel = remember(value) {
        ...
        if (bps <= 0) t("Unlimited", "نامحدود") else bps.toHumanSize() + "/s"
    }
    Column(verticalArrangement = Arrangement.spacedBy(2.dp)) {
        SettingsTextField(
            value         = value,
            onValueChange = onChange,
            label         = t("Max Speed (bytes/s, 0 = unlimited)", "حداکثر سرعت (بایت/ثانیه، ۰ = نامحدود)"),
            placeholder   = "0",
            icon          = Icons.Outlined.Speed,
            keyboardType  = KeyboardType.Number
        )
...
private fun AppearanceSection(context: android.content.Context) {
    Column(verticalArrangement = Arrangement.spacedBy(14.dp)) {
        ...
        Column(verticalArrangement = Arrangement.spacedBy(6.dp)) {
            Text(t("Language", "زبان"), color = TextSecondary, fontSize = 11.sp, fontWeight = FontWeight.SemiBold)
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                AppLanguage.entries.forEach { language ->
                    val selected = LanguageManager.current == language
                    Surface(
                        modifier = Modifier
                            .weight(1f)
                            .clickable { LanguageManager.setLanguage(context, language) },
                        shape  = RoundedCornerShape(10.dp),
                        color  = if (selected) CyanPrimary.copy(alpha = 0.14f) else ElevatedSurf,
                        border = BorderStroke(1.dp, if (selected) CyanPrimary else BorderColor)
                    ) {
                        Row(
                            ...
                            if (selected) {
                                Icon(Icons.Filled.Check, null, tint = CyanPrimary, modifier = Modifier.size(14.dp))
                                Spacer(Modifier.width(6.dp))
                            }
                            Text(
                                language.displayNameNative,
        ...
        Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Text(t("Theme", "پوسته"), color = TextSecondary, fontSize = 11.sp, fontWeight = FontWeight.SemiBold)
            AppThemeType.entries.forEach { theme ->
                val selected = ThemeManager.current == theme
                Surface(
                    modifier = Modifier
                        .fillMaxWidth()
                        .clickable { ThemeManager.setTheme(context, theme) },
                    shape  = RoundedCornerShape(12.dp),
                    color  = if (selected) theme.palette.elevatedSurf else theme.palette.surfaceDark,
                    border = BorderStroke(1.dp, if (selected) theme.palette.cyanPrimary else BorderColor)
                ) {
                    Row(
                        ...
                        Box(
                            modifier = Modifier
                                .size(38.dp)
                                .background(theme.palette.spaceBlack, RoundedCornerShape(9.dp))
                                .border(1.dp, theme.palette.borderColor, RoundedCornerShape(9.dp)),
                            contentAlignment = Alignment.Center
                        ) {
                            Row(horizontalArrangement = Arrangement.spacedBy(3.dp)) {
                                Box(Modifier.size(9.dp).background(theme.palette.cyanPrimary, CircleShape))
                                Box(Modifier.size(9.dp).background(theme.palette.purpleAccent, CircleShape))
                                Box(Modifier.size(9.dp).background(theme.palette.greenSuccess, CircleShape))
                            }
                        }
                        Spacer(Modifier.width(12.dp))
                        Column(Modifier.weight(1f)) {
                            Text(
                                t(theme.nameEn, theme.nameFa),
                                color = TextPrimary,
                                fontSize = 13.sp,
                                fontWeight = FontWeight.SemiBold
                            )
                            Text(
                                t(theme.descriptionEn, theme.descriptionFa),
                                color = TextMuted,
                                fontSize = 10.sp,
                                maxLines = 1,
                        ...
                        if (selected) {
                            Spacer(Modifier.width(6.dp))
                            Icon(Icons.Filled.CheckCircle, null, tint = theme.palette.cyanPrimary, modifier = Modifier.size(20.dp))
                        }
                    }
                }
            }
