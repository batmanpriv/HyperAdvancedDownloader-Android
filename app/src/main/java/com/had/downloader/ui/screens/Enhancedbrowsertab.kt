...
import com.had.downloader.service.DownloadConfig
...
@Composable
fun BrowserTabScreen(vm: MainViewModel, hlsDownloader: HlsDownloader) {
    ...
    Box(modifier = Modifier.fillMaxSize().background(SpaceBlack)) {
        Column(modifier = Modifier.fillMaxSize()) {
            ...
            BrowserTopBar(
                ...
                allTabs = state.tabs,
                onSelectTab = { id -> vm.switchBrowserTab(id) },
            ...
            BrowserBottomBar(
                ...
                onHome = {
                    ...
                    vm.updateBrowserTab(activeTab.id) { it.copy(url = "", title = t("New Tab", "برگه‌ی جدید")) }
...
private suspend fun resolveHlsVariantsWithRetry(
    hlsDownloader: HlsDownloader,
    url: String,
    pageTitle: String
): List<VideoStream> {
    val attempts = listOf(15_000, 30_000, 45_000) 
    var lastError: Exception? = null

    for ((index, timeoutMs) in attempts.withIndex()) {
        try {
            ...
            val streamsWithTitle = hlsDownloader.getVariantsWithQuality(url, config, pageTitle)
            if (streamsWithTitle.isNotEmpty()) {
                return streamsWithTitle.map { stream ->
                    VideoStream(
                        url = stream.url,
                        variantUrl = stream.variantUrl,
            ...
            lastError = e
            Log.w("HAD_Browser", "HLS variant lookup attempt ${index + 1}/${attempts.size} failed for $url: ${e.message}")
        }
    }

    Log.e("HAD_Browser", "All HLS variant lookup attempts failed for $url: ${lastError?.message}")
...
private fun detectFormatFromUrl(url: String): VideoFormat {
    val ext = url.substringAfterLast('.').substringBefore('?').lowercase()
    return when {
        url.contains(".m3u8") -> VideoFormat.HLS_M3U8
        url.contains(".mpd") -> VideoFormat.DASH_MPD
        ext == "mp4" || ext == "m4v" -> VideoFormat.MP4
        ext == "mkv" -> VideoFormat.MKV
        ext == "webm" -> VideoFormat.WEBM
        ext == "mp3" -> VideoFormat.AUDIO_MP3
        ext == "flac" -> VideoFormat.AUDIO_FLAC
        ext == "aac" -> VideoFormat.AUDIO_AAC
        else -> VideoFormat.UNKNOWN
    }
}

private fun isHlsSegment(url: String): Boolean {
    val lower = url.lowercase()
    return lower.contains(".ts") ||
            lower.contains("/segment_") ||
            lower.contains("/hls/") && !lower.contains(".m3u8") ||
            lower.matches(Regex("""/media_\d+\.ts""", RegexOption.IGNORE_CASE)) ||
            lower.matches(Regex("""/seg-\d+\.ts""", RegexOption.IGNORE_CASE)) ||
            lower.matches(Regex("""/\d+\.ts""", RegexOption.IGNORE_CASE))
}

private fun isDirectVideo(url: String): Boolean {
    val ext = url.substringAfterLast('.').substringBefore('?').lowercase()
    return ext in setOf("mp4", "mkv", "webm", "avi", "mov")
}

private fun extractQualityStreams(url: String, linkType: LinkType): List<VideoStream> {
    ...
    val qualities = listOf(
        "2160" to VideoQuality.UHD_4K,
        "1440" to VideoQuality.QHD_2K,
        "1080" to VideoQuality.FHD_1080,
        "720" to VideoQuality.HD_720,
        "480" to VideoQuality.SD_480,
        "360" to VideoQuality.SD_360,
        "240" to VideoQuality.LOW_240
    )

    val urlLower = url.lowercase()
    val detectedQuality = detectQualityFromUrl(url)

    if (detectedQuality != VideoQuality.UNKNOWN) {
        streams.add(
            VideoStream(
                url = url,
    ...
    for ((suffix, quality) in qualities) {
        if (quality == detectedQuality) continue

        val possibleUrl = when {
            url.contains("_${suffix}") -> url.replace("_${suffix}", "_${quality.label.lowercase()}")
            url.contains("-$suffix") -> url.replace("-$suffix", "-${quality.label.lowercase()}")
            url.contains("${suffix}p") -> url.replace("${suffix}p", "${quality.label.lowercase()}")
            else -> null
        }

        if (possibleUrl != null && possibleUrl != url) {
            streams.add(
                VideoStream(
                    url = possibleUrl,
                    quality = quality,
                    format = detectFormatFromUrl(possibleUrl),
                    linkType = linkType
                )
            )
        }
    ...
    return streams.sortedByDescending { it.quality.maxHeight }
}
...
private fun WebView.setupWebView(
    ...
    val shownDialogs = ConcurrentHashMap<String, Boolean>()
    ...
    webViewClient = object : WebViewClient() {
        ...
        override fun shouldInterceptRequest(
            ...
            if (isHlsSegment(url) || isLikelyNoise(url)) {
                return null
            }
            ...
            if (isDownloadable && isMediaExtension(url)) {
                ...
                val shouldShowDialog = (linkType == LinkType.VIDEO ||
                        linkType == LinkType.HLS ||
                        linkType == LinkType.DASH) &&
                        (url.contains(".m3u8") || url.contains(".mpd") || isDirectVideo(url))

                if (shouldShowDialog && !shownDialogs.containsKey(url)) {
                    shownDialogs[url] = true
                    Handler(Looper.getMainLooper()).post {
                        if (!vm.isVideoDialogShowing()) {
                            if (linkType == LinkType.HLS && url.contains(".m3u8")) {
                                
                                vm.showVideoDialogLoading(url)
                                scope.launch {
                                    val pageTitle = vm.browserUiState.value.activeTab.title
                                    val videoStreams = resolveHlsVariantsWithRetry(hlsDownloader, url, pageTitle)
                                    vm.updateVideoDialogStreams(url, videoStreams)
                                }
                            } else {
                                val streams = extractQualityStreams(url, linkType)
                                vm.showVideoDialogWithStream(url, streams)
                            }
                        }
                    }
                }
    ...
    addJavascriptInterface(object {
        ...
        fun intercept(url: String) {
            if (url.isNotBlank() && isMediaExtension(url) && !isHlsSegment(url) && !isLikelyNoise(url)) {
                val linkType = scraperEngine.classifyUrl(url)
                onIntercept(
                    InterceptedRequest(
                        url = url,
                ...
                if (!shownDialogs.containsKey(url) && (linkType == LinkType.VIDEO || linkType == LinkType.HLS || linkType == LinkType.DASH)) {
                    shownDialogs[url] = true
                    Handler(Looper.getMainLooper()).post {
                        if (!vm.isVideoDialogShowing()) {
                            if (linkType == LinkType.HLS && url.contains(".m3u8")) {
...
(function() {
  ...
  function isMedia(url) {
    ...
    var ext = lower.split('.').pop().split('?')[0];
    var mediaExts = ['mp4','mkv','webm','m3u8','mpd','mp3','flac','aac','m4a','zip','rar','7z','pdf','apk','torrent','ts','avi','mov','ogg','opus','epub','iso','dmg','exe','deb'];
...
private val noiseDomains = setOf(
    ...
    "moatads.com", "scorecardresearch.com", "adsrvr.org", "criteo.com",
    "taboola.com", "outbrain.com", "pubmatic.com", "rubiconproject.com",
    "media.net", "adnxs.com", "yieldmo.com", "casalemedia.com"
)

private val noiseKeywords = listOf(
    "vast", "vmap", "/ads/", "-ad.", "_ad.", "adbreak", "preroll", "midroll", "postroll",
    "bg-loop", "background-loop", "bgvideo", "bg_video", "hero-loop", "loading-spinner",
    "notification-sound", "ui-sound", "click-sound", "sprite", "/thumb/", "-thumb.",
    "tracking-pixel", "beacon", "telemetry", "/pixel/", "1x1.gif"
)

private fun isLikelyNoise(url: String): Boolean {
    val lower = url.lowercase()
    if (noiseDomains.any { lower.contains(it) }) return true
...
private fun BrowserTopBar(
    tab: BrowserTab,
    tabCount: Int,
    allTabs: List<BrowserTab> = emptyList(),
        ...
        Column {
            ...
            if (allTabs.size > 1) {
                LazyRow(
                    ...
                    items(allTabs, key = { it.id }) { t ->
                        val isActive = t.id == tab.id
                        Row(
                            modifier = Modifier
                                .clip(RoundedCornerShape(16.dp))
                                .background(if (isActive) CyanPrimary.copy(alpha = 0.18f) else ElevatedSurf)
                                .border(
                                    1.dp,
                                    if (isActive) CyanPrimary.copy(alpha = 0.6f) else BorderColor,
                                    RoundedCornerShape(16.dp)
                                )
                                .clickable { onSelectTab(t.id) }
                            ...
                            if (t.isLoading) {
                                ...
                                Icon(
                                    ...
                                    tint = if (isActive) CyanPrimary else TextMuted,
                                    modifier = Modifier.size(11.dp)
                                )
                            }
                            Text(
                                t.title.ifBlank { t("New Tab", "برگه‌ی جدید") },
            ...
            if (tab.title.isNotBlank() && tab.title != t("New Tab", "برگه‌ی جدید") && tab.url.isNotBlank()) {
                Text(
                    tab.title,
...
private fun BrowserHomePage(
    ...
    Column(
        ...
        Text(t("HAD Browser", "مرورگر HAD"), color = CyanPrimary, fontSize = 22.sp,
        ...
        OutlinedTextField(
            ...
            placeholder = { Text(t("Search or enter URL", "جستجو یا آدرس را وارد کن"), color = TextMuted) },
            keyboardOptions = androidx.compose.foundation.text.KeyboardOptions(
                imeAction = androidx.compose.ui.text.input.ImeAction.Go
            ),
            keyboardActions = androidx.compose.foundation.text.KeyboardActions(
        ...
        Row(
            ...
            Button(
                ...
                Text(t("Paste & Go", "چسباندن و رفتن"), fontSize = 12.sp, fontWeight = FontWeight.SemiBold)
            }
        }
            ...
            Surface(
                ...
                Row(
                    ...
                    Column(Modifier.weight(1f)) {
                        Text(t("$interceptedCount file${if (interceptedCount > 1) "s" else ""} detected", "$interceptedCount فایل شناسایی شد"),
        ...
        Text(t("QUICK ACCESS", "دسترسی سریع"), color = TextMuted, fontSize = 9.sp,
        ...
        Surface(
            ...
            Column(Modifier.padding(14.dp), verticalArrangement = Arrangement.spacedBy(7.dp)) {
                Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(6.dp)) {
                    ...
                    Text(t("Smart interception active", "شناسایی هوشمند فعال است"), color = CyanPrimary, fontSize = 11.sp, fontWeight = FontWeight.Bold)
                }
                listOf(
                    t("Browse any site — videos, files, music", "توی هر سایتی بگرد — ویدیو، فایل، موزیک"),
                    t("Content detected via WebView + JS bridge", "محتوا از طریق WebView و پل JS شناسایی می‌شود"),
                    t("Use Paste & Go to quickly open copied URLs", "از «چسباندن و رفتن» برای باز کردن سریع لینک‌های کپی‌شده استفاده کن"),
                    t("Tap ↓ button or download badge to download", "برای دانلود، روی دکمه‌ی ↓ یا نشان دانلود بزن")
                ).forEach { step ->
    ...
    Box(
        ...
        Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
            Row(
                ...
                Text(t("TABS (", "برگه‌ها (") + "${tabs.size})", color = TextPrimary, fontSize = 14.sp,
                ...
                Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                    Button(
                        ...
                        Text(t("New", "جدید"), fontSize = 12.sp, fontWeight = FontWeight.Bold)
                    }
...
private fun TabCard(tab: BrowserTab, isActive: Boolean, onSelect: () -> Unit, onClose: () -> Unit) {
    Surface(
        ...
        Row(
            ...
            Column(Modifier.weight(1f)) {
                Text(tab.title.ifBlank { t("New Tab", "برگه‌ی جدید") }, color = if (isActive) TextPrimary else TextSecondary,
...
fun InterceptedRequestsPanel(
    ...
    val filteredRequests = requests.filter {
        it.linkType != LinkType.FONT &&
                it.linkType != LinkType.API &&
                it.linkType != LinkType.OTHER &&
                it.linkType != LinkType.DIRECT
    }

    val grouped = remember(filteredRequests) {
        filteredRequests.groupBy { it.linkType }.toSortedMap(compareBy { it.name })
    }
            ...
            Column(Modifier.padding(16.dp)) {
                ...
                Row(modifier = Modifier.fillMaxWidth(),
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        ...
                        Text(t("Detected (", "شناسایی‌شده (") + "${filteredRequests.size})", color = TextPrimary, fontSize = 14.sp, fontWeight = FontWeight.Bold)
                    }
                    Row(horizontalArrangement = Arrangement.spacedBy(4.dp)) {
                        if (filteredRequests.isNotEmpty()) {
                            TextButton(onClick = { onDownloadAll(filteredRequests) }) {
