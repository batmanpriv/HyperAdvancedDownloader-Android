...
private fun WebView.setupWebView(
                            ...
                            if (linkType == LinkType.HLS && url.contains(".m3u8")) {
                                ...
                                scope.launch {
                                    try {
                                        ...
                                        val streamsWithTitle = hlsDownloader.getVariantsWithQuality(
                                            url,
                                            config,
                                            pageTitle
                                        )
                                        if (streamsWithTitle.isNotEmpty()) {
                                            val videoStreams = streamsWithTitle.map { stream ->
                                                VideoStream(
                                                    url = stream.url,
                                            ...
                                            vm.showVideoDialogWithStream(url, videoStreams)
                                        } else {
                                            ...
                                            vm.showVideoDialogWithStream(url, listOf(stream))
                                        }
                                    } catch (e: Exception) {
                                        val stream = VideoStream(
                                            url = url,
    ...
    addJavascriptInterface(object {
        ...
        fun intercept(url: String) {
            if (url.isNotBlank() && isMediaExtension(url) && !isHlsSegment(url)) {
                val linkType = scraperEngine.classifyUrl(url)
                onIntercept(
                    InterceptedRequest(
                        url = url,
                                ...
                                scope.launch {
                                    try {
                                        ...
                                        if (streamsWithTitle.isNotEmpty()) {
                                            val videoStreams = streamsWithTitle.map { stream ->
                                                VideoStream(
                                                    url = stream.url,                          // M3U8 اصلی
                                                    variantUrl = stream.variantUrl,            // ← واریانت انتخاب‌شده
                                                    quality = stream.quality,
                                                    format = stream.format,
                                                    sizeMb = stream.sizeMb,
                                                    bandwidth = stream.bandwidth,
