            ...
            if (torrentState.active.isEmpty() && torrentState.completed.isEmpty() && torrentState.failed.isEmpty()) {
                ...
                LazyColumn(
                    ...
                    if (torrentState.active.isNotEmpty()) {
                        item {
                            SectionLabel(t("ACTIVE  •  ", "فعال  •  ") + "${torrentState.active.size}", CyanPrimary)
                        }
                    ...
                    if (torrentState.completed.isNotEmpty()) {
                        item {
                            ...
                            SectionLabel(t("COMPLETED  •  ", "تکمیل‌شده  •  ") + "${torrentState.completed.size}", GreenSuccess)
                        }
                    ...
                    if (torrentState.failed.isNotEmpty()) {
                        item {
                            Spacer(Modifier.height(4.dp))
                            SectionLabel(t("FAILED  •  ", "ناموفق  •  ") + "${torrentState.failed.size}", RedError)
                        }
            ...
            AnimatedVisibility(visible = expanded) {
                Column {
                    ...
                    if (progress.files.isNotEmpty()) {
                        Text(
                            t("FILES (", "فایل‌ها (") + "${progress.files.size})",
                            color = TextMuted,
                            fontSize = 9.sp,
                            fontFamily = FontFamily.Monospace,
                            letterSpacing = 1.sp
...
private fun TorrentStatusBadge(status: String) {
    val (label, color) = when {
        ...
        status == "FETCHING_METADATA"  -> t("METADATA", "متادیتا")   to PurpleAccent
        status == "CONNECTING"         -> t("CONNECTING", "اتصال") to PurpleAccent
        status == "DOWNLOADING"        -> t("DL", "دانلود")         to CyanPrimary
...
private fun TorrentEmptyState(onAddTorrent: () -> Unit, onAddMagnet: () -> Unit) {
    Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
        Column(
            ...
            Text(
                t("Add a .torrent file or paste a magnet link", "یک فایل .torrent اضافه کن یا لینک مگنت رو بچسبون"),
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                Button(
                    ...
                    Text(t("Torrent File", "فایل تورنت"), fontWeight = FontWeight.Bold)
                }
                OutlinedButton(
                    onClick = onAddMagnet,
    ...
    AlertDialog(
        ...
        title = {
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Spacer(Modifier.width(8.dp))
                Text(t("Magnet Link", "لینک مگنت"), color = TextPrimary, fontWeight = FontWeight.Bold)
            }
        },
        text = {
            OutlinedTextField(
        ...
        confirmButton = {
            Button(
                ...
                Text(t("Start Download", "شروع دانلود"), fontWeight = FontWeight.Bold)
            }
        },
        dismissButton = {
    ...
    AlertDialog(
        ...
        text = {
            Column(modifier = Modifier.verticalScroll(rememberScrollState())) {
                Row(
                    ...
                    Text(
                        t("Select files to download", "فایل‌ها برای دانلود انتخاب کن"),
                ...
                if (info.comment.isNotBlank()) {
                    ...
                    Surface(
                        ...
                        Column(Modifier.padding(10.dp)) {
                            Text(t("Comment", "توضیح"), color = TextMuted, fontSize = 9.sp,
                                fontFamily = FontFamily.Monospace)
        ...
        confirmButton = {
            Button(
                ...
                Spacer(Modifier.width(4.dp))
                Text(t("Start (", "شروع (") + "${selectedSize.toHumanSize()})", fontWeight = FontWeight.Bold)
            }
        },
        dismissButton = {
            TextButton(onClick = onDismiss) { Text(t("Cancel", "انصراف"), color = TextSecondary) }
            ...
            Column(
                ...
                Surface(
                    ...
                    Column(Modifier.padding(12.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
                        DetailRow(t("Hash", "هش"), progress.infoHashHex, TextMuted)
                        DetailRow(t("Status", "وضعیت"), progress.status, CyanPrimary)
                        DetailRow(t("Downloaded", "دانلودشده"), progress.downloadedBytes.toHumanSize(), GreenSuccess)
                        DetailRow(t("Total", "کل"), progress.totalBytes.toHumanSize(), TextSecondary)
                        DetailRow(t("Speed", "سرعت"), progress.speedBps.toSpeedString(), CyanPrimary)
                        DetailRow(t("Peers", "پیرها"), "${progress.connectedPeers} " + t("connected", "متصل"), PurpleAccent)
                        DetailRow(t("Pieces", "قطعات"), "${progress.completedPieces}/${progress.pieces}", OrangeWarn)
                        if (progress.eta > 0) {
                            DetailRow(t("ETA", "زمان باقی‌مانده"), progress.eta.toEtaStr(), OrangeWarn)
                        }
                    }
                }

                if (progress.peers.isNotEmpty()) {
                    Text(t("PEERS (", "پیرها (") + "${progress.peers.size})", color = TextMuted,
                ...
                if (progress.files.isNotEmpty()) {
                    Text(t("FILES (", "فایل‌ها (") + "${progress.files.size})", color = TextMuted,
                        fontSize = 9.sp, fontFamily = FontFamily.Monospace, letterSpacing = 2.sp)
        ...
        confirmButton = {
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                Button(
                    ...
                    Text(t("Close", "بستن"), fontWeight = FontWeight.Bold)
                }
            }
        },
        dismissButton = {}
...
private fun PeerRow(peer: PeerInfo) {
    Row(
        ...
        Text(
            if (peer.downloadSpeed > 0) peer.downloadSpeed.toSpeedString() else t("idle", "بی‌کار"),
