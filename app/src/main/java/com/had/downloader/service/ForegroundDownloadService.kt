...
const val ACTION_DISMISS_ALL = "com.had.downloader.DISMISS_ALL"
...
class ForegroundDownloadService : Service() {
    ...
    fun updateForegroundNotif() {
        if (isNotifDismissed && activeIds.isEmpty()) {
            try { NotificationManagerCompat.from(this).cancel(NOTIF_FOREGROUND) } catch (_: Exception) {}
            return
        }
    ...
    private fun buildForegroundNotif(): Notification {
        ...
        val dismissAllPi = getService(
            this, 97,
            Intent(this, ForegroundDownloadService::class.java).apply { action = ACTION_DISMISS_ALL },
            FLAG_IMMUTABLE or FLAG_UPDATE_CURRENT
        )
        ...
        val builder = NotificationCompat.Builder(this, CHANNEL_DOWNLOAD)
            ...
            .setOngoing(activeCount > 0 || scheduledItems.isNotEmpty() || queuedItems.isNotEmpty())
        ...
        if (activeCount > 0) {
            ...
            if (activeItems.size == 1) {
                ...
                val etaStr = if (item.etaSeconds > 0) "  ⏱${item.etaSeconds.formatRemainingTime()}" else ""
            ...
            subText = buildString {
                if (scheduledItems.isNotEmpty()) {
                    append("⏰ ${scheduledItems.size} scheduled")
                }
            ...
            builder.addAction(
                NotificationCompat.Action.Builder(
                    android.R.drawable.ic_menu_close_clear_cancel, "Dismiss All", dismissAllPi
                ).build()
            )
            ...
            val scheduledInfo = buildString {
                if (sorted.size == 1) {
                    ...
                    append("${item.filename.take(20)}")
                    append(" at ${dateFormat.format(Date(epoch))}")
                    append(" • ${remainingStr} left")
                } else {
                    append("${sorted.size} downloads scheduled")
                    val firstEpoch = sorted.firstOrNull()?.second
                    val lastEpoch = sorted.lastOrNull()?.second
                    if (firstEpoch != null && lastEpoch != null) {
                        append(" (${SimpleDateFormat("HH:mm", Locale.getDefault()).format(Date(firstEpoch))} - ${SimpleDateFormat("HH:mm", Locale.getDefault()).format(Date(lastEpoch))})")
                    }
                }
            }

            builder.setContentTitle("⏰ Scheduled Downloads")
            builder.setContentText(scheduledInfo)
            builder.setProgress(0, 0, true)

            val queuedCount = queuedItems.filter { it.scheduleFrom.isBlank() }.size
            subText = if (queuedCount > 0) {
                "⏰ ${sorted.size} scheduled • 📋 $queuedCount queued"
            } else {
                "${sorted.size} item${if (sorted.size > 1) "s" else ""} waiting for schedule"
            }
            ...
            val regularQueued = queuedItems.filter { it.scheduleFrom.isBlank() }
            val count = regularQueued.size

            val fileNames = regularQueued.take(3).joinToString(", ") { it.filename.take(20) }
            ...
            subText = "Tap Start All to begin downloading"
            ...
            builder.addAction(
                NotificationCompat.Action.Builder(
                    android.R.drawable.ic_menu_close_clear_cancel, "Dismiss", dismissAllPi
                ).build()
            )
        }
