...
const val RETRY_WAIT_PREFIX = "RETRY_WAIT|"
...
class ForegroundDownloadService : Service() {
    ...
    private fun computeRetryBackoffMs(baseTimeoutSec: Int, attempt: Int): Long {
        val baseMs = baseTimeoutSec.coerceAtLeast(5) * 1000L
        val exponential = baseMs * (1L shl (attempt - 1).coerceIn(0, 4)) 
        val capped = exponential.coerceAtMost(180_000L) 
        val jitter = 1.0 + (Random().nextDouble() * 0.4 - 0.2) 
        return (capped * jitter).toLong().coerceAtLeast(3_000L)
    }
    ...
    private suspend fun handleDownloadFailed(id: Long, error: String) {
        ...
        if (item != null && nextRetryCount <= maxRetries && pendingRetries.add(id)) {
            ...
            val waitMs = computeRetryBackoffMs(timeoutSec, nextRetryCount)
            val resumeAt = System.currentTimeMillis() + waitMs

            dao.update(
                item.copy(
                    status = DownloadStatus.FAILED,
                    errorMessage = "$RETRY_WAIT_PREFIX$resumeAt|$nextRetryCount|$maxRetries|$error",
