...
private const val MAX_GLOBAL_RETRY_ROUNDS = 10
...
class SmartDownloader @Inject constructor(
    private val retryEngine: SmartRetryEngine
) {
    ...
    private suspend fun downloadFile(id: Long, config: DownloadConfig): DownloadResult {
        ...
        while (pendingChunks.isNotEmpty()) {
            ...
            if (failedChunks.isNotEmpty() && globalRetryAttempts < MAX_GLOBAL_RETRY_ROUNDS) {
                globalRetryAttempts++
    ...
    private suspend fun downloadChunkWithSmartRetry(
        ...
        var activeUserAgent = config.userAgent
        var activeHeaders = config.headers
        ...
        while (retries <= maxRetries) {
            ...
            try {
                ...
                val effectiveTimeoutMs = (config.timeoutMs + retries * 5_000).coerceAtMost(90_000)
                ...
                val conn = openConnection(url, attemptConfig, rangeStart, chunk.end)
                val responseCode = conn.responseCode

                if (chunk.downloaded > 0 && responseCode == 200) {
                ...
                if (responseCode != 200 && responseCode != 206) {
                    conn.disconnect()
                    throw HttpStatusException(responseCode, "HTTP $responseCode")
                }
                ...
                val statusCode = (e as? HttpStatusException)?.statusCode ?: 0
                ...
                synchronized(chunk) {
                    ...
                    if (statusCode == 416 || e.message?.contains("Requested Range") == true) {
                        chunk.downloaded = 0L
                    }
                }

                val decision = retryEngine.decide(
                    RetryContext(
                        downloadId = id,
                        url = url,
                        attempt = retries - 1,
                        ...
                        lastError = e.message,
                        currentUserAgent = activeUserAgent,
                        currentCookies = config.cookies,
                        currentHeaders = activeHeaders
                    )
                )
                retryEngine.logDecision(decision, id)
                decision.newUserAgent?.let { activeUserAgent = it }
                if (decision.additionalHeaders.isNotEmpty()) {
                    activeHeaders = activeHeaders + decision.additionalHeaders
                }

                val giveUp = retries > maxRetries || (statusCode != 0 && !decision.shouldRetry)
                if (giveUp) {
                    synchronized(chunk) {
                        chunk.status = ChunkStatus.FAILED
                        
                        chunk.retryCount = maxRetries + 1
                    }
                    Log.w(TAG, "Chunk ${chunk.index} giving up after $retries attempts: ${e.message}")
                    
                    return@withContext
                }

                val delayMs = decision.delayMs.takeIf { it > 0 } ?: exponentialDelayWithJitter(retries)
                delay(delayMs)
            }
        }
    }

    private fun exponentialDelayWithJitter(attempt: Int): Long {
        val base = (1_000L * (1L shl attempt.coerceIn(0, 5))).coerceAtMost(30_000L)
