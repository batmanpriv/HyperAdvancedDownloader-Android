...
class SmartDownloader @Inject constructor() {
    ...
    private suspend fun downloadFile(id: Long, config: DownloadConfig) {
        ...
        if (totalBytes <= 0L || !meta.acceptsRanges) {
            ...
            downloadSingleStreamWithSmartRetry(
                id = id,
                url = bestUrl,
                config = config,
        ...
        while (pendingChunks.isNotEmpty()) {
            ...
            val downloadJobs = remainingChunks.map { chunk ->
                parentScope.async {
                    downloadChunkWithSmartRetry(
                        ...
                        onSpeed = { bps ->
                            parentScope.launch {
                                lock.withLock { speedMap[chunk.index] = bps }
                            }
                        },
                        onProgress = {
                            ...
                            parentScope.launch {
                                emit(id, totalBytes, done, speed, eta, pct, chunks, currentThreads,
            ...
            if (failedChunks.isNotEmpty()) {
                globalRetryAttempts += failedChunks.size
                Log.d(TAG, "[$id] ${failedChunks.size} chunks failed, retrying in ${RETRY_INTERVAL_MS}ms... (attempt $globalRetryAttempts)")

                failedChunks.forEach { chunk ->
                    chunk.status = ChunkStatus.RETRYING
                    chunk.retryCount++
                    Log.d(TAG, "[$id] Chunk ${chunk.index} retry #${chunk.retryCount}")
                }
        ...
        if (!allDone) {
            ...
            if (stillFailed.isNotEmpty()) {
                Log.w(TAG, "[$id] ${stillFailed.size} chunks still failed after all retries, attempting full download...")
    ...
    private suspend fun downloadSingleStreamWithSmartRetry(
        ...
        while (retryCount < MAX_SMART_RETRIES && (totalBytes <= 0 || downloaded < totalBytes)) {
            ...
            try {
                val conn = openConnection(url, config, rangeStart = if (downloaded > 0) downloaded else -1L)
                    ...
                    outputStream.use { output ->
                        ...
                        while (input.read(buf).also { n = it } != -1) {
                            ...
                            if (delta >= SPEED_SAMPLE_MS) {
                                ...
                                if (stallCount >= 5) {
                                    Log.w(TAG, "[$id] Stalled detected, retrying...")
                                ...
                                emit(
                                    ...
                                    listOf(ChunkInfo(0, 0L, totalBytes - 1, downloaded, retryCount = retryCount)),
                                    1,
                ...
                if (totalBytes > 0 && downloaded >= totalBytes) {
                    Log.d(TAG, "[$id] Download completed despite error")
                    return
                }

                Log.w(TAG, "[$id] Single-stream attempt $retryCount failed: ${e.message}")

                if (retryCount >= MAX_SMART_RETRIES) {
                    Log.w(TAG, "[$id] Max retries reached for single-stream, starting from beginning...")
                    ...
                    try {
                        ...
                        freshConn.inputStream.use { input ->
                            fos.use { output ->
                                ...
                                while (input.read(buf).also { n = it } != -1) {
                                    ...
                                    output.write(buf, 0, n)
                                    downloadedFresh += n
                                    val pct = if (totalBytes > 0) downloadedFresh.toFloat() / totalBytes else 0f
                                    emit(
                                        id, totalBytes, downloadedFresh, 0L, -1, pct,
                                        listOf(ChunkInfo(0, 0L, totalBytes - 1, downloadedFresh, retryCount = retryCount)),
                                        1, "FALLBACK:${downloadedFresh}/${totalBytes}", mirrors, retryCount
                                    )
                                }
                            }
                        }
                        ...
                        Log.e(TAG, "[$id] Fallback download failed: ${fallbackError.message}")
                        throw fallbackError
                    }
                }
                ...
                if (e.message?.contains("416") == true || e.message?.contains("Requested Range") == true) {
                    Log.w(TAG, "[$id] Server doesn't support resume, starting from beginning...")
                    downloaded = 0L
                    outputFile.delete()
                }
            }
        ...
        if (totalBytes > 0 && downloaded < totalBytes) {
            throw IOException("Download failed after $retryCount attempts")
        }
    }
    ...
    private suspend fun downloadFullFileFallback(
        ...
        return withContext(Dispatchers.IO) {
            try {
                Log.d(TAG, "[$id] Starting full-file fallback download...")
                    ...
                    conn.inputStream.use { input ->
                        ...
                        while (input.read(buf).also { n = it } != -1) {
                            ...
                            if (delta >= SPEED_SAMPLE_MS) {
                                ...
                                val pct = if (totalBytes > 0) downloaded.toFloat() / totalBytes else 0f
                                emit(
                                    id, totalBytes, downloaded, speed, -1, pct,
                                    listOf(ChunkInfo(0, 0L, totalBytes - 1, downloaded)),
                                    1, "FALLBACK:${downloaded}/${totalBytes}", mirrors, 0
                                )
                            }
                        }
                    }
                ...
                Log.d(TAG, "[$id] Full-file fallback completed: $downloaded bytes")
                return@withContext true
            } catch (e: Exception) {
                Log.e(TAG, "[$id] Full-file fallback failed: ${e.message}")
                return@withContext false
            }
        }
    }
    ...
    private suspend fun downloadChunkWithSmartRetry(
        ...
        while (retries <= maxRetries) {
            ...
            try {
                    ...
                    conn.inputStream.use { input ->
                        ...
                        while (input.read(buf).also { n = it } != -1) {
                            ...
                            if (delta >= SPEED_SAMPLE_MS) {
                                ...
                                if (stallCount >= 5) {
                                    Log.w(TAG, "[$id] Chunk ${chunk.index} stalled, retrying...")
                ...
                onProgress()
                Log.d(TAG, "[$id] Chunk ${chunk.index} completed successfully")
                return@withContext
                ...
                Log.w(TAG, "[$id] Chunk ${chunk.index} attempt $retries failed: ${e.message}")

                if (e.message?.contains("416") == true || e.message?.contains("Requested Range") == true) {
                    Log.w(TAG, "[$id] Chunk ${chunk.index} - server doesn't support resume")
                    chunk.downloaded = 0L
                }

                if (retries > maxRetries) {
                    Log.e(TAG, "[$id] Chunk ${chunk.index} failed after $maxRetries attempts")
                    chunk.status = ChunkStatus.FAILED
